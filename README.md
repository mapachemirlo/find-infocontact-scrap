# Buscador de contactos por zona — Buenos Aires

Busca en **todos los buscadores disponibles** lo que vos quieras (ferreterías,
gimnasios, veterinarias, escuelas, estudios contables, etc.) en una zona del
Gran Buenos Aires (oeste, sur, este, norte), visita las páginas encontradas,
**extrae emails, teléfonos y redes sociales**, elimina lo repetido, y guarda lo
único en un **archivo .csv**.
...
| `zonas.py` | Localidades de cada zona (podés editarlas) |
...
```bash
# Ferreterías en zona oeste -> genera contactos_ferreterias_oeste.csv
python buscar_contactos.py --buscar "ferreterias" --zona oeste

# Gimnasios en zona norte, más resultados por búsqueda
python buscar_contactos.py --buscar "gimnasios" --zona norte --resultados 20

# Veterinarias en zona sur, con nombre de archivo propio
python buscar_contactos.py --buscar "veterinarias" --zona sur --salida vet_sur.csv

# Escuelas en zona este, sin la consulta extra de Google
...
| `--zona` | `oeste`, `sur`, `este` o `norte` | (pregunta) |
| `--resultados` | Páginas a revisar por cada búsqueda | 12 |
| `--salida` | Nombre del CSV de salida | `contactos_<termino>_<zona>.csv` |
